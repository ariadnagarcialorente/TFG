...
def main():
    ...
    # Iterate until all constraints are met
    iteration = 0
    while True:
        ...
        # Always prioritize row removal if possible
        if current_rows > min_rows_required:
            ...
            row_missing = current.loc[worst_row].isna().sum()
            row_candidate = ('row', worst_row, row_missing)
            
        # Consider column removal only if no row candidate and columns are removable
        if not row_candidate and current_cols > min_cols_required:
            ...
            if removable_cols:
                ...
                worst_col = col_missing.idxmax()
                col_missing_count = col_missing[worst_col]
                col_candidate = ('col', worst_col, col_missing_count)
        
        # Execute removal
        if row_candidate:
            _, row_id, row_missing_count = row_candidate
            current = current.drop(index=[row_id])
            current_missing -= row_missing_count
            if args.verbose:
                print(f'Iter {iteration}: Removed row {row_id} ({row_missing_count} missing)')
        elif col_candidate:
            _, col_id, col_missing_count = col_candidate
            current = current.drop(columns=[col_id])
            current_missing -= col_missing_count
            if args.verbose:
                print(f'Iter {iteration}: Removed col {col_id} ({col_missing_count} missing)')
        else:
            print("No candidates for removal but constraints not met")
            break
